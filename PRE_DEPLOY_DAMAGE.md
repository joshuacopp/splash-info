...
- [ ] **(Brief 138)** D1 schema migration applied: `idempotency_key TEXT` column on `claims` + partial unique index `idx_claims_idempotency_key`. Verify with `pnpm --filter @splash/damage-worker exec wrangler d1 execute splash-damage-claims --remote --command="PRAGMA table_info(claims);"` — the row list should include `idempotency_key TEXT`. The dedup path tolerates the column being missing (logged `[claim.idempotent] column missing — skipping dedup`), so apply-after-push is safe.
- [ ] **(Brief 138 offline banner)** Open `/claims/{site}` on a real customer device. DevTools → Network → set throttling to **Offline**. The amber **"You're offline — your form is saved on this device. Submit will retry automatically when the connection comes back."** banner should appear below the existing error banner. Re-enable network → the offline banner disappears within ~1s.
- [ ] **(Brief 138 retry-on-transient)** With DevTools network throttling set to **Offline** AND every required field filled (so client-side validation passes), click Submit. The submitting overlay should show **"Submitting (retry 1 of 3)..."** then **"...retry 2 of 3..."** then **"...retry 3 of 3..."** with 1s / 2s / 4s waits between attempts (total ~7s), then surface a final **"Network error: ... Please check your connection and retry."** banner. Alternatively, simulate a 503 via `wrangler dev` + a mocked PA / Supabase: the same overlay sequence should fire on HTTP 408 / 502 / 503 / 504 (deterministic 4xx like 400 / 403 / 422 do NOT retry).
- [ ] **(Brief 138 draft-survives-fail)** With the form filled but **before submitting**, set network throttling to Offline. Click Submit. Wait for the retry sequence to exhaust + the network-error banner to appear. Confirm in DevTools → Application → Local Storage that **`claims.draft.{site}` is still present** (Brief 136 + Brief 138 Phase 1). Reload the page → the amber **Resume** banner appears → click Resume → the typed customer-section fields rehydrate. Re-enable network → click Submit again → success.
- [ ] **(Brief 138 idempotency dedup)** Submit a successful claim from the form. Copy the response's `claim_id` and the `idempotency_key` payload field. Re-POST the same `FormData` (same `idempotency_key`) via curl or Postman: the response should carry the **same `claim_id`** plus `idempotent_replay: true`, and **NO** second row should be added to D1 (verify with a single-row count via `wrangler d1 execute splash-damage-claims --remote --command="SELECT COUNT(*) FROM claims WHERE idempotency_key='<key>';"`). Also verify that **no second customer-email webhook fires**, **no second PA POST**, **no second PDF gets generated** (check R2 for a single `claims/<claimId>/summary.pdf` object with the original Last-Modified time).
